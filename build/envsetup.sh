...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^noteng_") ; then
        NOTENG_BUILD=$(echo -n $1 | sed -e 's/^noteng_//g')
    else
        NOTENG_BUILD=
    fi
    export NOTENG_BUILD
...
function breakfast()
{
    target=$1
    local variant=$2
    source ${ANDROID_BUILD_TOP}/vendor/noteng/vars/aosp_target_release
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the Noteng model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch noteng_$target-$aosp_target_release-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/NotengOS_*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.noteng.device | grep -q "$NOTENG_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $NOTENG_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function githubremote()
{
    ...
    git remote add github https://github.com/NotengOS/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.noteng.device | grep -q "$NOTENG_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.noteng.device | grep -q "$NOTENG_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/noteng/build/tools/repopick.py $@
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $NOTENG_FIXUP_COMMON_OUT ]; then
